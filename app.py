...
def generate_from_transcript():
    """
    Browser fetches the transcript (bypassing cloud IP blocks),
    sends it here along with video metadata. Server does AI only.
    """
    data = request.json
    ...
    video_url = data.get("video_url", f"https://www.youtube.com/watch?v={video_id}")
    ...
    # Cache check
    cached = conn.execute(
        "SELECT * FROM notes WHERE video_id = ? AND style = ?", (video_id, style)
    ).fetchone()
    if cached:
        conn.close()
        return jsonify({
            "video_id": video_id, "title": cached["video_title"],
            "channel": cached["channel_name"], "thumbnail": cached["thumbnail"],
            "category": category, "style": style,
            "summary": cached["summary"],
            "key_takeaways": json.loads(cached["key_takeaways"] or "[]"),
    ...
    conn.execute("""
        ...
        (video_id, style, video_title, channel_name, thumbnail, summary, key_takeaways, how_to_apply, raw_transcript)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (video_id, style, title, channel, thumbnail, summary,
          json.dumps(takeaways), json.dumps(apply_steps), transcript[:5000]))
    conn.commit()
    conn.close()
